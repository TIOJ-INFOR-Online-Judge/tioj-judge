...
inline std::string CompileCodeName(CompileSubtask subtask, Compiler lang) {
  ...
  switch (subtask) {
    case CompileSubtask::USERPROG: return "prog" + extension;
    case CompileSubtask::SPECJUDGE: return "judge" + extension;
    case CompileSubtask::SUMMARY: return "summary" + extension;
    case CompileSubtask::PROBPROG: return "prob_prog" + extension;
  }
  __builtin_unreachable();
}
...
inline std::string InterlibName(int problem_id) { return "lib" + PadInt(problem_id, 4) + ".h"; }

inline fs::path BoxRoot(fs::path root, bool inside_box) { return inside_box ? fs::path("/") : root; }

} // namespace

fs::path SubmissionRunPath(long id) { return kBoxRoot / PadInt(id, 6); }
...
fs::path CompileBoxInput(long id, CompileSubtask subtask, Compiler lang, bool inside_box) {
  return Workdir(BoxRoot(CompileBoxPath(id, subtask), inside_box)) / CompileCodeName(subtask, lang);
}
...
fs::path CompileBoxOutput(long id, CompileSubtask subtask, Compiler lang, bool inside_box) {
  return Workdir(BoxRoot(CompileBoxPath(id, subtask), inside_box)) / CompileResultName(subtask, lang);
}
...
fs::path ExecuteBoxProgram(long id, int td, int stage, Compiler lang, bool inside_box) {
  return Workdir(BoxRoot(ExecuteBoxPath(id, td, stage), inside_box)) / ("prog" + ProgramExtension(lang));
}
fs::perms ExecuteBoxProgramPerm(Compiler lang, bool strict) {
  ...
  switch (lang) {
    ...
    case Compiler::CUSTOM:
      return fs::perms::owner_all | fs::perms::group_read | fs::perms::group_exec | fs::perms::others_read |
             fs::perms::others_exec; // 755
  }
  __builtin_unreachable();
}
...
fs::path ScoringBoxProgram(long id, int td, int stage, Compiler lang, bool inside_box) {
  return Workdir(BoxRoot(ScoringBoxPath(id, td, stage), inside_box)) /
         ("prog" + std::string(ProgramExtension(lang)));
}
fs::path ScoringBoxUserCode(long id, int td, int stage, Compiler lang, bool inside_box) {
  return Workdir(BoxRoot(ScoringBoxPath(id, td, stage), inside_box)) /
         ("code" + std::string(CodeExtension(lang)));
}
...
fs::path SummaryBoxPath(long id) { return SubmissionRunPath(id) / "summary"; }
...
fs::path DefaultScoringPath() { return internal::kDataDir / "default-scoring"; }
fs::path SpecjudgeHeadersPath() { return internal::kDataDir / "judge-headers"; }

fs::path SubmissionCodePath(int id) { return kSubmissionRoot / PadInt(id, 6); }
fs::path SubmissionUserCode(int id) { return SubmissionCodePath(id) / "prog"; }
fs::path SubmissionProblemProgCode(int id) { return SubmissionCodePath(id) / "prob_prog"; }
fs::path SubmissionJudgeCode(int id) { return SubmissionCodePath(id) / "judge"; }
fs::path SubmissionSummaryCode(int id) { return SubmissionCodePath(id) / "summary"; }
fs::path SubmissionInterlibCode(int id) { return SubmissionCodePath(id) / "interlib"; }
fs::path SubmissionInterlibImplCode(int id) { return SubmissionCodePath(id) / "interlib_impl"; }
